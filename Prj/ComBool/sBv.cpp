...
{ 
  ...
  ASSERT(m_nBitLength <= S_64);
  if (m_nBitLength == 0) 
    { m_bVect = 0; return;}
  m_bVect = Vect1 | (Vect2 >> (S_64 - Len2) << (S_64 - m_nBitLength));
}
...
{ CsBV s;
  s.Diz(bv.m_bVect, bv.m_nBitLength, 
        Vect >> (S_64 - bv.m_nBitLength) << (S_64 - bv.m_nBitLength), 
        bv.m_nBitLength);
  return s;
}
...
{ CsBV s;
  s.Diz(Vect >> (S_64 - bv.m_nBitLength) << (S_64 - bv.m_nBitLength), 
...
STD(CsBV) operator &(const ULONG Vect, const CsBV& bv)
{ CsBV s;
  s.Con(Vect >> (S_64 - bv.m_nBitLength) << (S_64 - bv.m_nBitLength), 
        bv.m_nBitLength, bv.m_bVect, bv.m_nBitLength);
  return s;
}
...
STD(CsBV) operator ^(const ULONG Vect, const CsBV& bv)
{ CsBV s;
  s.Add2(Vect >> (S_64 - bv.m_nBitLength) << (S_64 - bv.m_nBitLength), 
         bv.m_nBitLength, bv.m_bVect, bv.m_nBitLength);
  return s;
}
...
STD(CsBV) operator -(const ULONG Vect, const CsBV& bv)
{ CsBV s;
  s.ConNot(Vect >> (S_64 - bv.m_nBitLength) << (S_64 - bv.m_nBitLength), 
           bv.m_nBitLength, bv.m_bVect, bv.m_nBitLength);
  return s;
}
...
{ if (m_nBitLength != Len)  return FALSE;
  if (m_bVect != (Vect >>(S_64-m_nBitLength)<<(S_64-m_nBitLength))) return FALSE;
 return TRUE;
}
...
{ ASSERT(m_nBitLength == Len);
  ULONG V = Vect >> (S_64 - m_nBitLength) << (S_64 - m_nBitLength);
  if (Dist) { 
