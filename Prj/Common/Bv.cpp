...
{ size_t i,j=0;
  ...
  {
    if (GetBitAt(i)) res.SetAt((int)j,One);
    else             res.SetAt((int)j,Zero);
    if (Max!=0)
      if ((Max==1)||((i+1)%Max == 0))
      { res.SetAt((int)++j,'\n'); }  // res.SetAt(++j,'\r');
  }
...
// ASSERT((m_nBitLength - nNext) >= 0);
 ...
 for (j=i+1;j<(ptrdiff_t)m_nByteLength;j++)
   if (m_bVect[j])  {
...
{ptrdiff_t i;
 int pos;
 for (i=0;i<(ptrdiff_t)m_nByteLength;i++)
   if (m_bVect[i]!=0) {
     pos=0;
...
//***************************************************************************************
//-------------------------------------------------------------------
void CBV::AssignDiz(size_t nBitLength, const BYTE* v1, const BYTE* v2)
{
...
NewMem:
  ...
  for (size_t i=0; i<m_nByteLength; i++) m_bVect[i] = v1[i] | v2[i];
}
